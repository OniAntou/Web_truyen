 ...
 * @returns {HistoryEntry[]} Danh sách truyện đã đọc gần đây (mới nhất trước).
 */
export const getReadingHistory = (): HistoryEntry[] => {
    try {
 ...
 * @param {HistoryEntry} entry - { comicId, comicTitle, coverUrl, chapterId, chapterTitle, chapterNumber }
 */
export const saveReadingHistory = (entry: HistoryEntry) => {
    try {
