...
const ImagePreviewGrid: React.FC<ImagePreviewGridProps> = ({ images, setImages, showRemove = true, onRemove }) => {
    ...
    const handleRemove = (idx: number) => {
        ...
        const updated = [...images] as FilePreview[];
    ...
    return (
        <div className="mt-4">
            <div className="flex items-center justify-between mb-4">
                ...
                {showRemove && !onRemove && (
                    <button
                        type="button"
                        onClick={() => {
                            (images as FilePreview[]).forEach(img => URL.revokeObjectURL(img.preview));
                            setImages([]);
                        }}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {images.map((img, idx) => (
                    <div
                        key={(img as any).preview || (img as any).image_url || idx}
                        draggable
                    ...
                    >
                        <img
                            src={(img as any).preview || (img as any).image_url}
                            alt={`Page ${idx + 1}`}
...
const PreviewModal: React.FC<PreviewModalProps> = ({ isOpen, onClose, images, setImages, title, footerActions, allowAddMore = false, showRemove = true, onRemove }) => {
...
const ChapterManager: React.FC = () => {
    ...
    const [newChapter, setNewChapter] = useState<ChapterState>({
        chapter_number: '',
        title: '',
        price: '',
        early_access_end_date: ''
    });
    const [files, setFiles] = useState<FilePreview[]>([]); // array of { file, preview }
    ...
    const [existingChapterFiles, setExistingChapterFiles] = useState<FilePreview[]>([]);
    const targetChapterIdRef = useRef<string | null>(null);

    // For reordering existing chapter pages
    const [reorderModalOpen, setReorderModalOpen] = useState(false);
    const [reorderPages, setReorderPages] = useState<Page[]>([]); // { _id, page_number, preview }
    const [savingOrder, setSavingOrder] = useState(false);
    const reorderChapterIdRef = useRef<string | null>(null);
    ...
    const handleAddChapter = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFiles([]);
                const fileInput = document.getElementById('new-chapter-files') as HTMLInputElement;
                if (fileInput) {
                    fileInput.value = '';
                }
    ...
    const uploadImages = async (chapterId: string, fileList: File[]) => {
        ...
        const uploadBatchWithRetry = async (batch: File[], _batchIndex: number) => {
    ...
    const handleReorderClick = async (chapterId: string) => {
    ...
    const handleDeleteExistingPage = async (idx: number) => {
