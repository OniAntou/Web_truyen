        ...
        <div className="mt-8 space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-end pb-6 border-b border-white/5 gap-4">
                <div>
                    ...
                    <p className="text-zinc-300 mt-1 text-base font-medium">Manage and organize your comic collection.</p>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="flex bg-[#141414] border border-white/5 rounded-lg p-1 mr-2">
                        <button
                            ...
                            className={`p-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-white/10 text-white' : 'text-zinc-300 hover:text-zinc-100'}`}
                            title="List View"
                        >
                            <LayoutList size={20} />
                        </button>
                        <button
                            ...
                            className={`p-1.5 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white/10 text-white' : 'text-zinc-300 hover:text-zinc-100'}`}
                                    ...
                                    <tr key={comic._id || comic.id} className="hover:bg-white/[0.01] transition-colors duration-150 group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <button
                                                    onClick={() => handleDelete((comic._id || comic.id) as string)}
                        ...
                        <div key={comic._id || comic.id} className="group relative flex flex-col gap-3.5 bg-[#121212] p-4 rounded-xl border border-white/5 hover:border-white/10 transition-all">
                            <Link to={`/admin/comics/${comic._id || comic.id}/chapters`} className="aspect-[2/3] w-full rounded-lg overflow-hidden relative border border-white/5 block">
                                ...
                                <div className="absolute bottom-3 left-3 right-3 flex justify-between items-center text-white">
                                    <div className="flex gap-2 text-[0.65rem] font-medium">
                                ...
                                <div className="absolute top-3 right-3 px-2 py-1 bg-black/50 backdrop-blur-md rounded border border-white/10 text-[0.6rem] font-bold tracking-wider uppercase text-white">
                                    {comic.chapter_count || 0} Chaps
                                </div>
                            </Link>
                            
                            <button 
                                onClick={(e) => { e.preventDefault(); handleDelete((comic._id || comic.id) as string); }}
                                className="absolute top-6 left-6 p-2 bg-red-600/80 hover:bg-red-500 backdrop-blur-md rounded-lg text-white opacity-0 group-hover:opacity-100 transition-all shadow-lg z-10"
                                title="Delete Comic"
                            >
                                <Trash2 size={14} />
                            </button>
                            ...
                            <div className="space-y-3">
                                <h3 className="font-bold text-white text-sm line-clamp-1 group-hover:text-white transition-colors">{comic.title}</h3>
                                <div className="flex items-center justify-between gap-1">
                                    <span className={`px-2 py-0.5 rounded text-[0.55rem] font-bold border whitespace-nowrap ${comic.status === 'Ongoing' ? 'bg-emerald-500/5 text-emerald-500 border-emerald-500/10' :
                                            comic.status === 'Completed' ? 'bg-blue-500/5 text-blue-500 border-blue-500/10' :
                                ...
                                <div className="flex gap-2 pt-1">
                                    <Link to={`/admin/comics/${comic._id || comic.id}/chapters`} className="flex-1 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white transition-all text-[0.6rem] font-bold tracking-widest uppercase flex items-center justify-center gap-1.5 border border-white/5">
                                        <List size={14} /> Chapters
                                    </Link>
                                    <Link to={`/admin/comics/edit/${comic._id || comic.id}`} className="flex-1 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white transition-all text-[0.6rem] font-bold tracking-widest uppercase flex items-center justify-center gap-1.5 border border-white/5">
                                        <Edit2 size={14} /> Edit
                                    </Link>
                                </div>
                            </div>
