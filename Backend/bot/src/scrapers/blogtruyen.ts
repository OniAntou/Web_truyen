...
export class BlogTruyenScraper implements Scraper {
  ...
  canHandle(url: string): boolean {
    return /blogtruyen(vn)?\.\w+/i.test(url);
  }
  ...
  async scrapeMeta(url: string): Promise<ComicMeta> {
    ...
    try {
      ...
      return await page.evaluate(() => {
        ...
        const genreNodes = document.querySelectorAll('a[href*="the-loai"], .category a, .genres a, [itemprop="genre"] a');
    ...
    try {
      ...
      return await page.evaluate(() => {
        ...
        document.querySelectorAll(
          'img[alt*="chapter"], .page-image img, #content img, .chapter-content img, .reading img, .page-chapter img'
        ).forEach((img) => {
