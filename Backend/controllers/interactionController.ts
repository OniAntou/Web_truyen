import {  Comic, Rating, ComicView, Comment, Favorite, ReadingProgress, Chapter, Pages  } from "../Database/database";
...
const getComments = asyncHandler(async (req, res) => {
  ...
  if (chapterId) {
    filter.chapter_id = String(chapterId);
  } else {
...
const getUserFavorites = asyncHandler(async (req, res) => {
  ...
  const results = await Promise.all(comics.map(async (c) => {
    const coverUrl = await resolveR2Url((c as any).cover_url);
    return {
      ...c,
      cover_url: coverUrl || (c as any).cover_url,
...
const getReadingProgress = asyncHandler(async (req, res) => {
  ...
  res.json({ 
    ...
    chapter_number: (progress.chapter_id as any).chapter_number,
    chapter_title: (progress.chapter_id as any).title,
...
const getAllReadingProgress = asyncHandler(async (req, res) => {
  ...
  for (const progress of progresses) {
    ...
    const coverUrl = await resolveR2Url((progress.comic_id as any).cover_url);
    results.push({
      comic_id: progress.comic_id._id,
      comic_id_legacy: (progress.comic_id as any).id,
      comic_title: (progress.comic_id as any).title,
      comic_cover: coverUrl || (progress.comic_id as any).cover_url,
      chapter_id: progress.chapter_id._id,
      chapter_number: (progress.chapter_id as any).chapter_number,
      chapter_title: (progress.chapter_id as any).title,
      page_number: progress.page_number,
      updated_at: progress.updated_at,
      genres: (progress.comic_id as any).genres
    });
  }
