...
app.get('/api/genres', async (req, res) => {
    try {
        ...
        // If a specific genre is requested, return its comics
        let comics = [];
        if (genre) {
...
app.get('/api/comics/latest', async (req, res) => {
    try {
        ...
        // Genres for filter
        const allComics = await Comic.find();
        const allGenres = [...new Set(allComics.flatMap(c => c.genres || []))].sort();
...
app.get("/api/comics/popular", async (req, res) => {
  try {
    ...
    // Collect all unique genres for filter options
    const allComics = await Comic.find();
    const allGenres = [
      ...new Set(allComics.flatMap((c) => c.genres || [])),
    ].sort();
...
app.post("/api/chapters/bulk-delete", async (req, res) => {
  try {
    ...
    if (!chapterIds || !Array.isArray(chapterIds)) {
      return res
        .status(400)
        .json({ message: "Invalid payload: chapterIds must be an array" });
    }
